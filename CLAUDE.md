# Relayterm project instructions for Claude Code

These instructions apply to Claude Code working anywhere in this repository. More specific instruction files in subdirectories may extend or override them for their scope.

`CLAUDE.md` and `AGENTS.md` intentionally contain the same shared project policy so that different agent tools receive equivalent guidance. Keep their shared rules aligned when either file changes.
