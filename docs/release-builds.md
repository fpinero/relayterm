...
An initial native macOS repetition produced equal binaries and archives, but a subsequent privacy inspection found native build-user paths in dependency diagnostics. Those artifacts were rejected. Source `491d5f1037450c362525b289fa6361d834fc0b6f` includes path remapping and private-prefix rejection. Quality run `34947043434` built and packaged that source twice on all three native targets. Each target produced byte-identical executable copies and byte-identical normalized archive copies, and each privacy inspection passed. The exact sizes, SHA-256 values, native job links, and the separate local macOS build are recorded in the [candidate handoff](m12-candidate-handoff.md). These observations prove byte identity only within each declared native runner and input set. They do not claim cross-host reproducibility.
...
The inspected Windows x64 PE imports `VCRUNTIME140.dll` and the Windows Universal CRT API sets. This candidate therefore requires a supported Microsoft Visual C++ v14 Redistributable for x64 that is at least as recent as the MSVC build tools used for the artifact. Microsoft recommends the latest supported matching-architecture package in its [Visual C++ Redistributable guidance](https://learn.microsoft.com/en-us/cpp/windows/latest-supported-vc-redist?view=msvc-170). Relayterm does not download or install that system component. Execution on a hosted Windows Server runner with the runtime already present does not establish a clean Windows 10 installation; that remains part of the installed-candidate observation.
...
The inspected macOS executable is Mach-O arm64 with deployment target 11.0. It links `/usr/lib/libiconv.2.dylib` and `/usr/lib/libSystem.B.dylib`; SQLite is bundled. The declared macOS 14 floor remains the oldest tested runtime, regardless of the lower linker deployment field. The extracted smoke passed on macOS 14 in CI and on macOS 26.5.2 locally with only `/usr/bin:/bin` on `PATH` for the product process. A short private test root under `/tmp` was required because Unix-domain socket paths are length-bounded. Production default private locations are compact; an excessively long explicit `--home` can fail with `daemon_unavailable` and should be replaced with a shorter private path.
