...
The automated native baseline starts a real foreground `rt` daemon over real SQLite and local IPC. It creates a task and local Git worktree, registers and launches a neutral synthetic agent through a real PTY, reconnects through repeated authenticated clients, then creates and restores a private backup after stopping the daemon. During the live workflow it samples both daemon and agent process network endpoints. Linux correlates `/proc/<pid>/fd` socket inodes with the process network tables, macOS uses `lsof -a -p <pid> -i`, and Windows uses a bounded test-only PowerShell adapter that calls the native `GetExtendedTcpTable` and `GetExtendedUdpTable` owner-table APIs directly. Before inspecting Relayterm, the same monitor must detect a test-only loopback TCP listener. The monitor observes the long-lived product processes, while the separate Git hardening tests establish that Relayterm invokes only local Git operations with network transports disabled. The target runs twice on each stable native runner:
