...
The `hardening_gate` target composes two isolated native workspaces. Its TUI scenario exercises live input, resize, history, detach and reattach with three sessions, and its worktree scenario exercises real TUI creation and launch, task coordination, worktree identity and durable restart. The retained durable slice releases two independent native clients through an explicit barrier to race one ready-task claim and verifies exactly one winner. Fault injection remains in separately named targets so each boundary has bounded setup, teardown and failure reporting. Candidate-specific native, resource, offline and manual results are reconciled in the acceptance matrix.
...
The M11 branch investigated all eleven inherited risks:

- R1: a Git child can exit while a descendant retains its stdout handle. On Unix, each Git invocation now runs in an isolated process group, both output readers use nonblocking descriptors, and completion terminates remaining members before joining bounded captures. Linux stable exposed `EIO` when the residual group was terminated after the Git parent exited. The reader now observes confirmed command completion before group cleanup, retries an interrupted read, and treats a subsequent pipe error as closure; errors while the command is live remain failures. Final review found that the silent status and worktree-add helpers waited boundedly but did not terminate their Unix process groups after completion. Candidate `d6efd7f` routes both helpers through bounded wait and residual-group termination. The Unix regression covers captured output, status, and quiet paths; Windows retains Job Object containment for every path. CI passed two independent serial repetitions on every applicable stable target.
- R2: Git runs with a cleared and explicit environment, disabled system/global attributes and configuration, prompts, pagers, credentials, network transports and lazy fetch. Every Git operation overrides hooks, file-monitor commands and external attribute files. Worktree creation rejects filter configuration plus selected-commit nested attributes before checkout. Inert regressions verify that common and destination-specific included hooks, file monitors, a credential helper and an included filter do not execute. No Relayterm Git operation requests a remote. Candidate `d6efd7f` passed the complete native Git matrix, including two isolated descendant-containment repetitions on every stable operating system.
- R3: destination, branch and task reservations are revalidated while holding a native lock derived from the canonical Git common directory. The cross-process regression opens the same repository from independent processes and proves that only one contender can add the reserved branch and destination. Durable intent identity precedes the external effect, and a conflict is never adopted as another operation's receipt.
- R4: creation verification still pins the initial commit, while later launch verification checks repository, common directory, branch and checkout identity without requiring HEAD to remain at that initial commit. The native Git regression creates a user commit in the linked checkout, accepts ordinary subsequent verification and confirms that the stricter creation proof would reject the changed HEAD.
- R5: NUL-delimited porcelain parsing rejects duplicate and malformed records without trimming identity text. Tests cover spaces, significant line whitespace, Unicode, Unix non-UTF-8 repository paths, Windows drive, UNC and verbatim adaptation, portable reserved names and reparse rejection. Unsupported representations fail before `worktree add`; identity-bearing paths are never lossy-converted.
- R6: Git inspection and creation have separate active and queued admission limits. A test-only barrier cancels the client both before `worktree add` and after the external effect but before SQLite finalization. The stable operation ID remains queryable, and explicit read-only reconciliation finalizes confirmed state without issuing another add or destructive compensation.
- R7: launch admission resolves task, definition, selected worktree, executable and immutable argument snapshot against one expected revision before spawning. Existing TUI definition-race regressions preserve drafts and reject stale mutations, worktree tests reject cross-task selection and clearing a worktree used by a live instance, and the installed-layout restore gate migrates a populated pre-worktree database without source-tree migrations. The persisted instance and session return the same admitted identifiers and working directory used for launch.
- R8: native CLI harnesses cap captured output, child polling, reader completion and cleanup. Tests cover oversized lines and inherited writers, and required CI invocations are separate steps. The repository check includes a negative mutation that proves a combined step would be rejected. The Windows offline monitor was changed from an unbounded management-provider query to deadline-controlled native `netstat.exe` processes after the provider stalled a native CI job.
- R9: backup reserves a private output inside a uniquely owned staging directory before `VACUUM INTO`, validates the captured database, writes the bounded manifest last and uses a native no-replace directory publication. The manifest handle is closed after synchronization and before directory publication because Windows rejects renaming a directory that contains this open handle. SQLite tests exercise a concurrent WAL writer, cancellation and full-write recovery; CLI tests cover aliases, existing destinations, interrupted manifests, checksum changes, structural corruption, logical cross-row corruption, old and new schemas and preservation of originals. Backup admission rejects a second concurrent request immediately.
- R10: named-terminal observations are mandatory in both the TUI guide and acceptance matrix. Automated PTY and ConPTY evidence remains distinct and cannot close Linux terminal, Terminal.app, Windows Terminal or SSH cells.
- R11: snapshot collection responses previously loaded all definitions, tasks, instances, claims, progress, handovers and worktrees before applying item and byte limits. Every public collection now uses revision-checked SQLite pages with `LIMIT + 1`, stable opaque-ID cursors and the existing response byte limit. Task and instance pages reconstruct child records only for the admitted page, and filtered worktree pages retain the same revision contract. Point reads and mutation transactions use an entity projection derived from the command or observation. The projection includes directly referenced entities, nonfinal instances, open claims, direct dependencies and the explicitly capped worktree state, while immutable progress and unrelated closed history remain in SQLite. Revision and uniqueness checks still run inside the write transaction. Backup and restore validate foreign keys and cross-row state with scalar queries, then decode events in 200-row pages. The scale gate exercises 10,000 tasks plus 100,000 progress records and matching events, preserves 100,001 progress rows after a new mutation, and returns 200-item pages. Candidate `d6efd7f` passed two repetitions on Linux, macOS and Windows with native p95 query times between 2 and 4 ms and mutations between 1 and 11 ms.
...
These findings and their automated native evidence apply to candidate `d6efd7f`. The manual matrix and final technical reconciliation are complete, as recorded in the [M11 final technical review](m11-final-review.md). The owner approved delivery and PR #22 is merged. Post-merge CI passed and the [M12 handoff](m12-handoff.md) is prepared. M11 is complete.
