...
- R11: snapshot collection responses previously loaded all tasks, progress and handover rows before applying item and byte limits. These collections now use revision-checked SQLite pages with `LIMIT + 1`, stable opaque-ID cursors and the existing response byte limit. Task pages reconstruct dependency, scope, owner and worktree context for at most the admitted page. Persistence and real IPC regressions cover page boundaries, next cursors and stale revisions. Definition, instance, claim, worktree and mutation snapshots still reconstruct larger workspace working sets, so R11 and the 10,000-task or 100,000-history workload remain pending.
