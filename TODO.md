...
- M12.NATIVE: Reconcile Linux correction-candidate acceptance while retaining the 9cf91f7 baseline, affected macOS header/writer-size physical retests on the prepared 691a8fb installation (see docs/m12-macos-correction-candidate.md), and the focused SSH observation through an authorized connection. Reconcile clean-machine and global acceptance evidence. Preserve the Windows source mapping in docs/m12-windows-manual-observations.md; use the existing Linux task prompt in docs/m12-native-continuation.md.
