...
- M12.CORRECTION-PLATFORMS: On candidate 691a8fb, reconcile acceptance of the demonstrated SSH transport-loss recovery and fixed-grid history limits recorded in docs/m12-linux-final-report.md; preserve the original failed observations.
  - M12.WINDOWS-DEBUG-LATENCY: Establish the cause and resolve or explicitly reconcile the reproduced Windows debug input misses against 250 ms. Follow docs/m12-windows-final-delta.md: seven of eight focused debug attempts fail, release passes separately, and the finite-flood versus sustained-load coverage gap also needs reconciliation. Preserve original 290/294 ms failures and every new attempt; no product fix or waiver is established.
...
- M12.NATIVE: Complete independent macOS and Windows clean-runtime installation and applicable automated quick-start evidence using their retained final packages. Reuse completed physical observations. Follow the finite remaining requirements in docs/m12-linux-final-report.md; preserve every platform's source/hash mapping.
  - M12.WINDOWS-INDEPENDENT-RUNTIME: Provide an identified independent native Windows boundary and execute the prepared installed procedure with enforced positive/negative access controls. The available developer host passed its disposable functional journey but retains absolute checkout/compiler access; see docs/m12-windows-final-delta.md. Do not close this task using PATH filtering or a fresh home alone.
  - M12.MAC-HANDOFF-RECONCILIATION: Reconcile the maintainer-supplied completed Mac runtime and final SSH presentation with the newer Mac-local reports and older queue entries. See docs/m12-windows-final-delta-continuation.md; do not repeat completed physical or SSH tests because those reports were unavailable on Windows.
