...
- M12.CORRECTION-PLATFORMS: Retest the inherited 15794ad corrections and informational feedback on candidate 691a8fb on macOS/Windows: disconnected indicators, release chord navigation, bounded diagnostics, and rename context after review. Reconcile acceptance of the demonstrated SSH transport-loss recovery and fixed-grid history limits recorded in docs/m12-linux-final-report.md; preserve the original failed observations.

- M12.NATIVE: Reconcile Linux correction-candidate acceptance while retaining the 9cf91f7 baseline, affected macOS writer-size physical retests on the prepared 691a8fb installation (see docs/m12-macos-correction-candidate.md), and the focused SSH observation through an authorized connection. Reconcile clean-machine and global acceptance evidence. Preserve the Windows source mapping in docs/m12-windows-manual-observations.md; use the existing Linux task prompt in docs/m12-native-continuation.md.
